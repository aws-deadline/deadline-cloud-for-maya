...
def installer_path():
    path = "DeadlineCloudForMayaSubmitter-{platform}-installer.{ext}"
...
def _validate_files(installation_path: Path) -> None:
    ...
    python_dir = installation_path / "scripts"
    ...
    module_dir = [f.name for f in (python_dir).iterdir()]
...
def test_default_location(installer_path: Path):
    ...
    --prefix <prefix>                           Installation Directory
                                                Default: /home/<user>/DeadlineCloudForMayaSubmitter
    """
    # GIVEN
    default_install_location = Path("~/DeadlineCloudForMayaSubmitter").expanduser()
